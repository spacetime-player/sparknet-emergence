...
TOTAL_STEPS = 25000   # run full 10k steps
...
class SparkNet(nn.Module):
    def __init__(self, n):
        ...
        # dimming
        self.register_buffer("inactive_counter", torch.zeros(n, dtype=torch.long))
    ...
    def step(self):
        ...
        # ===== dimming unused weights =====
        fired_mask = fired
        self.inactive_counter[fired_mask] = 0
        self.inactive_counter[~fired_mask] += 1

        dim_mask = self.inactive_counter > DECAY_START_STEPS
        if dim_mask.any():
            self.W.data[dim_mask, :] *= (1 - DECAY_RATE)
