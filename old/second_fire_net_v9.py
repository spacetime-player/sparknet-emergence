...
def setup_plots():
    ...
    fig.subplots_adjust(top=0.78)   # push plots down to make space for stats
...
def update_plots(net, fig, s_img, w_img, cycle, step, stats):
    ...
    # fresh text HUD
    fig.text(0.02, 0.97, f"Cycle: {cycle}/{TOTAL_CYCLES}", fontsize=10)
    fig.text(0.02, 0.94, f"Step:  {step}/{CYCLE_LENGTH}", fontsize=10)
    fig.text(0.02, 0.91, f"New: {new_sparks}   Fired: {fired}   Alive: {alive}", fontsize=10)
